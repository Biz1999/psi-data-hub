...
class ListYesterdayProductsController {
  async handle(estoque = "S", page: number) {
    try {
      const apikey = process.env.API_KEY;
      const yesterdayFormatted = format(subDays(new Date(), 1), "dd/MM/yyyy");
      const encoded = encodeURI(
        `/produtos/page=${page}/json&apikey=${apikey}&estoque=${estoque}&filters=dataAlteracao[${yesterdayFormatted} TO ${yesterdayFormatted}]`
      );
...
export { ListYesterdayProductsController };
